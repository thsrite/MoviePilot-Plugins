...
class SiteStatisticNoMsg(_PluginBase):
    # 插件名称
    plugin_name = "站点数据统计"
    # 插件描述
    plugin_desc = "自动统计和展示站点数据（无站点未读消息）。"
    # 插件图标
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "sitestatisticnomsg_"
    # 加载顺序
    plugin_order = 1
    # 可使用的用户级别
    auth_level = 2
    ...
    def init_plugin(self, config: dict = None):
        ...
        if self._enabled or self._onlyonce:
            # 加载模块
            self._site_schema = ModuleHelper.load('app.plugins.sitestatisticnomsg.siteuserinfo',
