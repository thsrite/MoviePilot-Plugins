...
from app.sdk.cache import FileCache
...
class EmbyMetaRefresh(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        self._tmdb_cache = FileCache(
            base=self.get_data_path() / "tmdb_cache",
            ttl=604800
        )
                            ...
                            if str(item.get('Type')) == 'Episode':
                                if item.get("Id") in self._episodes_images:
                                    ...
                                    if not tv_info:
                                        ...
                                        series_info = emby.get_iteminfo(series_id) if series_id else None
                                        tmdb_id = self.__get_tmdb_media_id(series_info)
                                        if tmdb_id:
                                            tv_info = self.tmdbapi.get_info(
                                                tmdbid=tmdb_id,
                                                mtype=MediaType.TV,
                                            )
                                    if not tv_info:
    ...
    @staticmethod
    def __get_tmdb_media_id(media) -> Optional[int]:
        """读取统一媒体身份中的 TMDB ID。"""
        media_source, media_id = resolve_media_identity(media)
        if media_source == MediaSource.TMDB and media_id and media_id.isdigit():
            return int(media_id)
        return None

    @staticmethod
    def __media_identity_mismatch(left, right) -> bool:
        """仅在两边具有相同来源时判断媒体身份冲突。"""
        left_source, left_id = resolve_media_identity(left)
        right_source, right_id = resolve_media_identity(right)
        return bool(
            left_source
    ...
    def __refresh_emby(self, transferinfo, emby):
        ...
        try:
            if transferinfo.type == "电影":
                ...
                # 同一媒体来源下 ID 不一致时，说明命中了同名的其他媒体。
                item_info = emby.get_iteminfo(item_id)
                if self.__media_identity_mismatch(transferinfo, item_info):
                    logger.error(
                        f"Emby中{transferinfo.title} ({transferinfo.year})的媒体ID与入库记录不一致"
                    )
                    return
