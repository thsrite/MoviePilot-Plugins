...
class SubscribeGroup(_PluginBase):
        ...
        if self._update_confs:
            ...
            for confs in str(self._update_confs).split("\n"):
                ...
                for conf in str(confs).split("#"):
                    if ":" in conf:
                        k = conf.split(":", 1)[0].strip().lower()
                        v = conf.split(":", 1)[1].strip()
                        if k == "category":
                            category = v
                        if k == "resolution":
                            resolution = v
                        ...
                        if k == "sites":
                            for site_name in str(v).split(","):
                                for active_site in active_sites:
                                    if site_name.strip().casefold() == str(active_site.name).casefold():
                ...
                if category:
                    for c in str(category).split(","):
                        self._subscribe_confs[c.strip().casefold()] = {
            ...
            logger.info(f"获取到二级分类自定义配置 {len(self._subscribe_confs)} 个")
    ...
    def subscribe_notice(self, event: Event = None):
        ...
        if event:
            ...
            if not category:
                ...
                logger.info(f"订阅ID:{sid} 未获取到二级分类，尝试通过媒体信息识别 {media_info}")
            ...
            if category_key not in self._subscribe_confs:
            ...
            if not subscribe:
                logger.error(f"订阅ID:{sid} 对应订阅记录不存在")
                return

            # 二级分类自定义配置
            category_conf = self._subscribe_confs[category_key]
    ...
    def download_notice(self, event: Event = None):
        ...
        if event:
            ...
            logger.info(
                f"获取到媒体 {media_key} season {season} 订阅记录:{len(subscribes)} 个")
            ...
            for subscribe in subscribes:
                ...
                if "站点" in self._update_details and (
                    ...
                    # 站点 判断是否在订阅站点范围内
                    rss_sites = {str(site_id) for site_id in self.systemconfig.get(SystemConfigKey.RssSites) or []}
                    if _torrent and _torrent.site and str(_torrent.site) in rss_sites:
