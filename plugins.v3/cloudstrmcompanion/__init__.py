...
class CloudStrmCompanion(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        if self._enabled or self._onlyonce:
            ...
            # 读取目录配置
            monitor_confs = (self._monitor_confs or "").split("\n")
        ...
        for emby_name, emby_server in emby_servers.items():
            ...
            try:
                ...
                if res and res.status_code in [200, 204]:
                    ...
                    logger.error(f"通知媒体服务器 {emby_name} 刷新新增文件 {strm_file} 失败，错误码：{status_code}")
                    return False
    ...
    def export_dir(self, fid, destination_id="0"):
        ...
        request = RequestUtils(headers=self._headers)
    ...
    @eventmanager.register(EventType.PluginAction)
    def scan_command(self, event: Event = None):
        """处理远程全量同步命令。"""
        if not event:
            return
        ...
        for medis_title_year_season in list(self._medias.keys()):
            ...
            if (datetime.now() - last_update_time).total_seconds() > int(self._interval) \
                ...
                if self._notify:
                    ...
                    # 获取封面图片
                    media_source, media_id = resolve_media_identity(media=file_meta)
                    recognize_kwargs = {
                        "meta": file_meta,
                        "mtype": media_type,
                    }
                    ...
                    mediainfo: MediaInfo = self.chain.recognize_media(**recognize_kwargs)
